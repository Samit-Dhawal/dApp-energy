    ...
    return (
        <div>
            ...
            <form style={styles.formOnly} onSubmit={(evt) => updateWallet(evt)}>
                {error ? (<><label style={{ color: 'red', fontWeight: 800, fontSize: 20 }}>Error : Wallet not updated</label><br /></>) : (<></>)}

                <label style={styles.label}>Add Money:</label><br />
                <input type="text" name="wallet" style={styles.input} onChange={(text) => setNewWallet(text.target.value)} value={newWallet} placeholder='Add money to your wallet' size="40" />
                <br />
                <br />
